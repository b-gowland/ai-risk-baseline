...
export function buildMarkdownExport(output, config, today = new Date()) {
  ...
  lines.push(``)
  lines.push(`**Generated:** ${dateStr} · [AI Risk Practice Baseline](https://b-gowland.github.io/ai-risk-baseline/)`)
  lines.push(``)
  ...
  lines.push(`> [Full disclaimer →](https://b-gowland.github.io/ai-risk-baseline/disclaimer)`)
  lines.push(``)
  ...
  if (config.context?.domain) lines.push(`- **Domain:** ${config.context.domain}`)
  if (config.context?.who) lines.push(`- **Users:** ${config.context.who}`)
  if (config.context?.decision_mode) lines.push(`- **Decision mode:** ${config.context.decision_mode}`)
  ...
  if (output.isBroad) {
    ...
    lines.push(`> [Refine your configuration →](https://b-gowland.github.io/ai-risk-baseline/configure)`)
    lines.push(``)
  }
  ...
  if (output.risks?.length) {
    ...
    output.risks.forEach((r, i) => {
      ...
      if (r.kb_ref) {
        lines.push(``)
        lines.push(`[View in Knowledge Base →](https://b-gowland.github.io/ai-risk-kb/docs/${r.kb_ref})`)
      }
      lines.push(``)
    })
  }
  ...
  lines.push(`---`)
  lines.push(``)
  lines.push(`*Generated by [AI Risk Practice Baseline](https://b-gowland.github.io/ai-risk-baseline/) · Apache 2.0 open source · Content CC BY 4.0*`)
